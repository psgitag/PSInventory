...
function App() {
  ...
  const API_URL =
    "https://script.google.com/macros/s/AKfycbzJbVy3ZnM5gXLgRJVnjhQrpTxFJRUIUO1JnAn_63ZyYgs1YktXRYuCw3IBMpm43x0y/exec";
                ...
                processedItems.map((item, index) => (
                  <tr key={index} style={{ borderBottom: "1px solid #eee" }}>
                    {Object.entries(item).map(([key, val], i) => {
                      // Convert key to lowercase and remove spaces/underscores for maximum flexibility
                      const cleanKey = key.toLowerCase().replace(/[\s_]/g, "");
                      const isLink = String(val).startsWith("http");

                      return (
                        <td key={i} style={{ fontSize: "14px", color: "#555" }}>
                          {/* Checks if the column header has "image" in it and contains a valid web link */}
                          {cleanKey.includes("image") && isLink ? (
                            <button
